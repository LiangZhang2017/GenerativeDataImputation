...
class InfoGAN:
    def __init__(self,tensor,parameters):
        ...
        Version is:
            ...
            'WithTensor'=>1
        '''
        self.InfoGAN_mode = 'Standard' # 'Standard','WithTensor'
        self.Version = np.nan
    ...
    def RunModel(self,model_iter):
        ...
        sub_tensors, self.output_size = Partition(self.tensor, slice_size=self.slice_size, mode="Average",
                                                  filter='normal')
        ...
        if self.slice_size != self.tensor.shape[0]:
            ...
            for fold, ((train_set, test_set), (train_index, test_index)) in enumerate(k_fold_split(sub_tensors, n_splits=5)):
                ...
                if self.InfoGAN_mode=='Standard':
                    ...
                    generator=InfoGAN_Standard_Generator(channel=train_set_tensor.shape[-1],output_size=self.output_size,input_shape=input_shape)

                    discriminator=InfoGAN_Standard_Discriminator(self.cont_code1_dim, self.cont_code2_dim)
                    ...
                    while not converge:
                        with tf.GradientTape() as gen_tape, tf.GradientTape() as disc_tape:
                           ...
                           fake_out,fake_cont_out1,fake_cont_out2 = discriminator(fake_data,self.is_training)
                           real_out,real_cont_out1,real_cont_out2 = discriminator(train_set_tensor,self.is_training)
                           ...
                           # Calculate MAE
                           mae = tf.reduce_mean(tf.abs(train_set_tensor[mask_train_x] - fake_data[mask_train_x]))

                           # Calculate MSE
                           mse = tf.reduce_mean(tf.square(train_set_tensor[mask_train_x] - fake_data[mask_train_x]))

                           # Calculate RMSE by taking the square root of MSE
                           rmse = tf.sqrt(mse)

                           # Compute RSE
                           # Calculate the mean of the actual values for masked elements
                           mean_actual = tf.reduce_mean(train_set_tensor[mask_train_x])

                           # Calculate the total squared difference from the actual values to the mean of the actual values
                           variance_actual = tf.reduce_mean(tf.square(train_set_tensor[mask_train_x] - mean_actual))

                           # Compute RSE
                           rse = mse / variance_actual

                           true_labels = train_set_tensor[mask_train_x]
                           predictions = fake_data[mask_train_x]

                           # Initialize the AUC metric object
                           ...
                           cross_entropy=bce_loss(train_set_tensor[mask_train_x],fake_data[mask_train_x])

                           '''
                           Generator Loss
                           '''
                           # Mutual Information Loss using the discriminator's continuous codes predictions
                           ...
                           gen_loss=fake_loss_g+cont1_loss+cont2_loss+train_rmse
                           ...
                           disc_loss=real_loss+fake_loss_d+cont1_loss+cont2_loss
                        ...
                        print(epoch, "d-loss:", float(disc_loss), "g-loss:", float(gen_loss), "RMSE:", rmse)

                        train_perf.append([mae, rmse, rse, auc_score, cross_entropy])
                    ...
                    test_mae_loss = tf.losses.mean_absolute_error(test_set_tensor[mask_test_x],test_fake_data[mask_test_x])
                    test_mae_loss = tf.reduce_mean(test_mae_loss)

                    test_mse_loss = tf.losses.mean_squared_error(test_set_tensor[mask_test_x],test_fake_data[mask_test_x])
                    ...
                    test_fake_data = tf.cast(test_fake_data, tf.float32)

                    # Calculate MAE
                    test_mae = tf.reduce_mean(tf.abs(test_set_tensor[mask_test_x] - test_fake_data[mask_test_x]))

                    # Calculate MSE
                    test_mse = tf.reduce_mean(tf.square(test_set_tensor[mask_test_x] - test_fake_data[mask_test_x]))
                    ...
                    auc_metric = tf.keras.metrics.AUC()
                    auc_metric.update_state(test_set_tensor[mask_test_x], test_fake_data[mask_test_x])
                    test_auc = auc_metric.result().numpy()

                    bce_loss = tf.keras.losses.BinaryCrossentropy(from_logits=True)
                    cross_entropy = bce_loss(test_set_tensor[mask_test_x],test_fake_data[mask_test_x])

                    try:
                        test_auc_score = roc_auc_score(test_set_tensor[mask_test_x], test_fake_data[mask_test_x])
                    ...
                    test_perf.append([test_mae, test_rmse, test_rse_loss, test_auc_score, cross_entropy])
                    ...
                    test_set_tensor_reshape = tf.squeeze(test_fake_data, axis=1)
            ...
            return cv_results, fold_summary
                ...
                # if self.InfoGAN_mode=='WithTensor':
                #     self.Version = 1
                #
                #     existing_original_tensor=train_set_tensor[mask_train_x]
                #
                #     preparation=InfoGANInputPreparationWithTensor(self.noise_dim, self.cont_code1_dim, self.cont_code2_dim)
                #     combined_input = preparation([train_set_tensor, batch_size])
                #
                #     generator = InfoGAN_Standard_Generator_WithTensor(channel=train_set_tensor.shape[-1],
                #                                            output_size=self.output_size, input_shape=input_shape)
                #
                #     discriminator = InfoGAN_Standard_Discriminator(self.cont_code1_dim, self.cont_code2_dim)
                #
                #     g_optimizer = keras.optimizers.Adam(learning_rate=self.learning_rate, beta_1=0.5)
                ...
                #     converge = False
                #     epoch = 0
                #     test_mae_loss, test_rmse_loss, test_auc_score = 0., 0., 0.
                #
                #     original_tensor, noise, cont_code1, cont_code2=combined_input
                #
                #     while not converge:
                #         with tf.GradientTape() as gen_tape, tf.GradientTape() as disc_tape:
                #             fake_data = generator(combined_input, self.is_training)
